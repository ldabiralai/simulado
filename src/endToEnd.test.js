...
describe('Simulado', () => {
  describe('Start', () => {
    ...
    it('starts on port 7001 by default', () => {
      Simulado = require('./index');
      Simulado.start();

      return portInUse(7001).then(portInUse => {
        expect(portInUse).to.equal(true);
      });
    });

    it('starts on a custom port number', () => {
      Simulado = require('./index');
      Simulado.start({ port: 1234 });

      return portInUse(1234).then(portInUse => {
        expect(portInUse).to.equal(true);
      });
    });
  });
